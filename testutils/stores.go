...
func PrepareStores(ctx context.Context, t *testing.T, db repositories.Execer) error {
	t.Helper()

	err := CleanUpStores(ctx, t, db)
	if err != nil {
		return err
	}

	err = CreateStores(ctx, t, db)
	if err != nil {
		return err
	}

	err = InsertStores(ctx, t, db)
	if err != nil {
		return err
	}
...
func CleanUpStores(ctx context.Context, t *testing.T, db repositories.Execer) error {
	t.Helper()

	const sqlStr = `DROP TABLE IF EXISTS stores;`
...
func CreateStores(ctx context.Context, t *testing.T, db repositories.Execer) error {
	t.Helper()

	const sqlStr = `CREATE TABLE IF NOT EXISTS stores (
...
func InsertStores(ctx context.Context, t *testing.T, db repositories.Execer) error {
	t.Helper()

	const sqlStr = `INSERT INTO stores (store_cd, company_cd, store_name, address, latitude, longitude)
	VALUES
